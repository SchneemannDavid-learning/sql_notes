####### RAVINDER'S FUNCTIONS LESSON NOTES, COPIED FROM GIT
...
####### MY NOTES FOLLOWING ALONG WITH ABOVE:
...
from employees;
	...
	# if you wanted to case convert list:
select lower(concat(first_name,' ', last_name)) as full_name
from employees;

 # we can also concatenate columns with other pieces of string
select lower(concat(first_name, '_', last_name, '@company.com')) as email_address
from employees;
...
select substr(first_name, 2, 10)
from employees;


# create a username for employees using firstletter of first_name, an underscore and lastname

select lower(concat(substr(first_name, 1, 1), '_', last_name)) as username
from employees;
...
# we can get the day or month name of the week from a date (or string that matches the date)
select month('2022-05-19');
select monthname('2022-05-19');
select dayname('2022-05-19');

select *, dayname(hire_date) as day_hired, dayname(birth_date) as day_of_birth
from employees;

select year(hire_date) as h_year, month(hire_date) as h_month 
from employees;
...
from employees;
	...
	# if you'd like to round the decimals..
select round(datediff(curdate(), hire_date)/365, 1) as tenure
from employees;
